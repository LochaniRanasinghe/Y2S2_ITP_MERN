...
function AllLeaves() {
  ...
  const getLeaves = () => {
    axios
      .get("http://localhost:8070/empLeave/")
      .then((res) => {
        setLeaves(res.data);
      })
      .catch((err) => {
  ...
  function deleteLeave(id) {
    swal({
      ...
      if (willDelete) {
        ...
        axios
          .delete("http://localhost:8070/empLeave/delete/" + id)
          .then((res) => {
          ...
          {leaves
            .filter((val) => {
              if (searchDoc === "") {
                ...
                val.leaveID.toLowerCase().includes(searchDoc.toLowerCase()) ||
            ...
            .map((data) => {
              return (
                <tr>
                  <td>{data.leaveID}</td>
                  <td>{data.empID}</td>
                  <td>{data.dateOfLeave}</td>
                  <td>{data.dateOfReturn}</td>
                  <td>{data.approvalStatus}</td>
                  <td>
                    <Link
                      to={"/empLeave/update/" + data._id}
